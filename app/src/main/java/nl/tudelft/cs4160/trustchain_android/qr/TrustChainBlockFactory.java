...
public class TrustChainBlockFactory {
    public MessageProto.TrustChainBlock createBlock(QRBlock qrBlock, QRTransaction transaction, MessageProto.TrustChainBlock previousBlock, PublicKey linkPublicKey, KeyPair ownKeyPair) {
        ...
        builder.setTransaction(ByteString.copyFromUtf8(walletAdapter.toJson(transaction)))
                .setPublicKey(ByteString.copyFrom(ownKeyPair.getPublicKey().toBytes()))
